...
Part of the add-venue-profile skill (research-paper-skills); vendored from
parse-cfp's fetch_cfp.py so this skill stays self-contained when installed
on its own. Stdlib only. Used to extract venue-profile facts from the live
CFP — every number in a profile must come from fetched text, never memory.
...
Politeness contract (do not weaken):
  ...
  - responses cached under .cache/add-venue-profile/ (gitignored), 24h reuse
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Politely fetch ONE conference CFP page, cache it under "
                    ".cache/add-venue-profile/, and print readable text to stdout. "
                    "Every fact in a venue profile must come from fetched text "
                    "like this, never from memory.",
        epilog="examples:\n"
    ...
    ap.add_argument("--cache-dir", type=Path,
                    default=Path(os.environ.get("CFP_CACHE_DIR", ".cache/add-venue-profile")),
                    help="cache directory (default: .cache/add-venue-profile, gitignored)")
