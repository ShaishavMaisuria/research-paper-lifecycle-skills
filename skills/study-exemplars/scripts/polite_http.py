#!/usr/bin/env python3
"""Shared plumbing for the study-exemplars scripts. Python 3 stdlib only.
...
Provides:
  ...
  - http_get(): polite GET with a per-host rate limit persisted across
    ...
    and response caching under .cache/study-exemplars/ (gitignored).

This module is not a CLI. It is imported by rank_top_cited.py and
lookup_exemplar.py, which live in the same directory.
"""
...
CACHE_DIR = os.path.join(os.getcwd(), ".cache", "study-exemplars")
RATELIMIT_FILE = os.path.join(CACHE_DIR, "_ratelimit.json")
DEFAULT_TTL = 24 * 60 * 60  # seconds; citation counts move at most daily
...
def _throttle(host: str, min_interval: float) -> None:
    ...
    Timestamps persist in .cache/study-exemplars/_ratelimit.json so
    back-to-back script invocations are throttled too, not just calls
    within one process.
    """
...
def http_get(
    ...
    *,
    ...
    - serves from .cache/study-exemplars/ when a fresh (< ttl) entry exists
    ...
    - with none_on_404=True, an HTTP 404 returns None (caller decides the
      exit code — "paper not found" is a different failure than a network
      error); any other failure exits nonzero with a clear message
    """
