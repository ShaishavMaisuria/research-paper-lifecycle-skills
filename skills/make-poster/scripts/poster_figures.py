#!/usr/bin/env python3
"""Inventory a LaTeX paper's figures for poster reuse and pick hero candidates.

Scans the paper's LaTeX source (a main .tex file; \\input/\\include files are
followed) and reports every figure environment: graphics files, labels,
caption snippets, and how often each figure is referenced in the text — the
reference count is a strong signal for which figure should anchor the
poster (the "hero" figure that is readable from 3 m away). Resolves
graphics paths the way LaTeX does (\\graphicspath, extension-less names
tried as .pdf/.png/.jpg/.jpeg/.eps) and can copy the resolved files into a
poster-assets directory.

Stdlib only. No network.

Usage:
    python3 poster_figures.py paper/main.tex
    python3 poster_figures.py paper/main.tex --copy-to poster/assets
    python3 poster_figures.py paper/main.tex --json
...
def classify_roles(figures):
    """Hero = most-referenced figure with a resolvable graphics file."""
    usable = [f for f in figures
              if f["graphics"] and all(g["resolved"] for g in f["graphics"])]
    usable.sort(key=lambda f: -f["refs"])
    roles = {}
    if usable:
        roles[usable[0]["index"]] = "HERO candidate"
        for f in usable[1:4]:
            roles[f["index"]] = "supporting"
    for f in figures:
        if not f["graphics"]:
            roles[f["index"]] = "TikZ/pgf-only — recompile standalone or crop from the paper PDF"
        elif f["index"] not in roles:
            roles.setdefault(
                f["index"],
                "unresolved graphics" if not all(g["resolved"] for g in f["graphics"])
                else "spare — use only if space allows")
    return roles
...
def print_markdown(figures, n_tables, roles, main_path):
    print("# Poster figure inventory — %s" % main_path)
    print()
    if not figures:
        print("No figure environments found. If the paper keeps figures in a")
        print("separate file, pass the file that contains them.")
        return
    print("| # | label(s) | refs | graphics file | resolved | poster role | caption (trimmed) |")
    print("|---|----------|------|---------------|----------|-------------|-------------------|")
    for fig in sorted(figures, key=lambda f: -f["refs"]):
        ...
        cap = fig["caption"][:70] + ("…" if len(fig["caption"]) > 70 else "")
        print("| %d | %s | %d | %s | %s | %s | %s |"
              % (fig["index"], labels, fig["refs"], gfx, ok,
                 roles.get(fig["index"], ""), cap or "(no caption)"))
    print()
    print("- %d figure environment(s); %d table environment(s) — tables do not"
          % (len(figures), n_tables))
    print("  survive on posters: redesign each as a chart or one huge number.")
    print("- The HERO candidate (most-referenced, printable) should anchor the")
    print("  poster: biggest element after the title, readable from 3 m.")
    print("- Paper figures carry paper-sized fonts: re-export with axis labels")
    print("  and legends >= 24 pt at print size, or they vanish at 1.5 m.")


def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Inventory a LaTeX paper's figures for poster reuse "
        "(labels, captions, graphics files, in-text reference counts, "
        "hero-figure candidates); optionally copy the graphics into a "
        "poster-assets directory.")
    ...
    if args.json:
        json.dump({"figures": figures, "tables": n_tables,
                   "roles": roles, "missing": missing}, sys.stdout, indent=2)
        sys.stdout.write("\n")
    else:
        print_markdown(figures, n_tables, roles, args.main_tex)
        if args.copy_to is not None:
