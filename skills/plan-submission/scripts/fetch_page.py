...
Part of the plan-submission skill (research-paper-skills); vendored from
parse-cfp's fetch_cfp.py so this skill stays self-contained. Stdlib only.
Used to re-verify live CFP deadlines before a submission plan is trusted.
...
Politeness contract (do not weaken):
  ...
  - responses cached under .cache/plan-submission/ (gitignored), 24h reuse
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Politely fetch ONE conference CFP page, cache it under "
                    ".cache/plan-submission/, and print readable text to stdout. "
                    "Use it to re-verify deadlines on the live CFP before "
                    "trusting a submission plan.",
        epilog="examples:\n"
               "  CONTACT_EMAIL=you@uni.edu python3 fetch_page.py "
               "https://sigspatial2026.sigspatial.org/research-submission.html\n"
               "  python3 fetch_page.py <url> --html      # raw HTML instead of text\n"
               "  python3 fetch_page.py <url> --refresh   # bypass the 24h cache",
    ...
    ap.add_argument("--cache-dir", type=Path,
                    default=Path(os.environ.get("CFP_CACHE_DIR", ".cache/plan-submission")),
                    help="cache directory (default: .cache/plan-submission, gitignored)")
